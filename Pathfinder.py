...
def backTrackerMaze(number, width=101, height=101):
	shape = (height,width)
	# Build actual maze
	Z = np.ones(shape, dtype=bool) # Maze-grid: 1's are black, 0's are whit
	...
	for arrFile in os.listdir("arrs/randGrid/"):
		...
		while(arr[goalCol][goalRow]==1):
			if(i==width*height*2):
				...
				print("Probabilistically very unlikely")
				i=width*height
				break
			goalCol,goalRow = rr.choice(range(0,height)), rr.choice(range(0,width))
			i=i+1;
		...
		plt.xticks([]), plt.yticks([])
		plt.savefig("pics/randGrid/forwardBigG{0:0=2d}.png".format(i))
		np.savetxt("arrs/randGrid/forwardBigG{0:0=2d}.txt".format(i),arrTemp,fmt='%d')
		plt.close('all')
		#start forward A star with small g
		arrTemp = copy.deepcopy(arr);
		agentCol=startCol
		agentRow=startRow
		start = (startCol, startRow)
		...
		while True:
			...
			if len(tieList)>1:
				for j in range(len(tieList)-1):
					if(tieList[j][1]>tieList[j+1][1]):
						tieList[j], tieList[j+1] = tieList[j+1], tieList[j]
			smallest = tieList.pop(0)
			heapq.heappush(closed, smallest)
			for tie in tieList:
				heapq.heappush(heap, tie)
			item = smallest
			if(arrTemp[item[3][0]][item[3][1]]==2):
		...
		plt.imshow(arrTemp, cmap, interpolation='nearest')
		plt.xticks([]), plt.yticks([])
		plt.savefig("pics/randGrid/forwardSmallG{0:0=2d}.png".format(i))
		np.savetxt("arrs/randGrid/forwardSmallG{0:0=2d}.txt".format(i),arrTemp,fmt='%d')
		#start backward A star
		arrTemp = copy.deepcopy(arr);
		agentCol=goalCol
		agentRow=goalRow
		goal= (agentCol, agentRow)
		closed=[]
		h=0
		g=abs(startCol-goalCol)+abs(startRow-goalRow)
		f=g+h
		costToReach=0
		parentSpot = start
		goalData = (f, g, h, goal, 0, goal)
		heapq.heappush(closed, goalData)
		heap = []
		#check above wall
		checkRow=agentRow
		checkCol=agentCol+1
		if(checkCol<height and checkCol>=0):
			if(arrTemp[checkCol][checkRow]!=1):
				spot = (checkCol, checkRow)
				if arrTemp[checkCol][checkRow]!=5:
						arrTemp[checkCol][checkRow]=2
				h=abs(checkCol-goalCol)+abs(checkRow-goalRow)
				g=abs(checkRow-startRow)+abs(checkCol-startCol)
				f=g+h
		...
		while True:
			...
			if(checkCol<height and checkCol>=0 and (inClose==False)):
				if(arrTemp[checkCol][checkRow]!=1):
					...
					hData = (f, g, h, spot, costToReach, parentSpot)
					heapq.heappush(heap, hData)
		startSpot = (startCol, startRow)
		for thing2 in closed:
			if(thing2[3]==startSpot):
				while thing2[5]!=thing2[3]:
					agentCol = thing2[5][0]
					agentRow = thing2[5][1]
					agentSpot = (agentCol, agentRow)
					if(arrTemp[agentCol][agentRow]!=4):
						arrTemp[agentCol][agentRow]=6
					for space in closed:
						if(space[3]==agentSpot):
							thing2 = space
							break
		#print graph for backward A star
		plt.figure()
		colorArr = ['white', 'black', 'green', 'red', 'orange', 'yellow', 'blue']
		leg = plt.legend(['Empty', 'Wall', 'Open', 'Closed', 'start', 'goal', 'Optimal Path'])
		for i, j in enumerate(leg.legendHandles):
			j.set_color(colorArr[i])
		cmap = colors.ListedColormap(['white', 'black', 'green', 'red', 'orange', 'yellow', 'blue'])
		plt.imshow(arrTemp, cmap, interpolation='nearest')
		plt.xticks([]), plt.yticks([])
		plt.savefig("pics/randGrid/backward{0:0=2d}.png".format(i))
		np.savetxt("arrs/randGrid/backward{0:0=2d}.txt".format(i),arrTemp,fmt='%d')
		plt.close('all')
		#now run adaptive A star
		arrTemp = copy.deepcopy(arr);
		agentCol=startCol
		agentRow=startRow
		start = (startCol, startRow)
		...
		plt.imshow(arrTemp, cmap, interpolation='nearest')
		plt.xticks([]), plt.yticks([])
		plt.savefig("pics/randGrid/adaptStar{0:0=2d}.png".format(i))
		np.savetxt("arrs/randGrid/adaptStar{0:0=2d}.txt".format(i),arrTemp,fmt='%d')
		plt.close('all')
